...
export class ClaudeProcessManager extends EventEmitter {
  ...
  private findClaudeExecutable(): string {
    ...
    throw new Error(
      'Claude executable not found in node_modules. Ensure @anthropic-ai/claude-code is installed.',
    );
  }
  ...
  async startConversation(
    ...
    if (isResume && !workingDirectory && config.resumedSessionId) {
      const fetchedWorkingDirectory = await this.historyReader.getConversationWorkingDirectory(
  ...
  async stopConversation(streamingId: string): Promise<boolean> {
    ...
    try {
      ...
      if (!process.killed) {
        this.logger.debug('Process still running, sending SIGTERM', {
          streamingId,
          pid: process.pid,
        });
        ...
        const killTimeout = setTimeout(() => {
          if (!process.killed) {
            this.logger.warn('Process not responding to SIGTERM, sending SIGKILL', {
              streamingId,
              pid: process.pid,
            });
  ...
  async waitForSystemInit(streamingId: string): Promise<SystemInitMessage> {
    ...
    return new Promise<SystemInitMessage>((resolve, reject) => {
      ...
      const processClosedHandler = ({
        streamingId: closedStreamingId,
        code,
      }: {
        streamingId: string;
        code: number | null;
      ...
      const messageHandler = ({
        ...
        const missingFields = requiredFields.filter(
          (field) => systemInitMessage[field] === undefined,
        );
        ...
        if (missingFields.length > 0) {
          ...
          reject(
            new CUIError(
              'INCOMPLETE_SYSTEM_INIT',
              `System init message missing required fields: ${missingFields.join(', ')}`,
              500,
            ),
          );
          return;
        ...
        if (this.conversationStatusManager && config) {
          ...
          this.logger.debug(
            'Registered active session with status tracker (no optimistic service)',
            {
              streamingId,
              claudeSessionId: systemInitMessage.session_id,
            },
  ...
  private async executeConversationFlow(
    ...
    try {
      ...
      const delayPromise = new Promise<string>((resolve) => {
        setTimeout(() => {
          this.logger.debug('Spawn validation period passed, process appears stable', {
            streamingId,
            ...loggerContext,
          });
      ...
      this.logger.debug(
        `${operation.charAt(0).toUpperCase() + operation.slice(1)} conversation successfully`,
        {
          streamingId,
          sessionId: systemInit.session_id,
  ...
  private buildResumeArgs(config: {
    sessionId: string;
    message: string;
  ...
  private buildStartArgs(config: ConversationConfig): string[] {
    this.logger.debug('Building Claude start args', {
      ...
      promptPreview: config.initialPrompt
        ? config.initialPrompt.substring(0, 50) + (config.initialPrompt.length > 50 ? '...' : '')
        : null,
      workingDirectory: config.workingDirectory,
      model: config.model,
    });
    ...
    try {
      ...
      claudeProcess.on('error', (error: Error & NodeJS.ErrnoException) => {
        this.logger.error('Claude process spawn error', error, {
          ...
          errorSpawnargs: (error as Error & NodeJS.ErrnoException & { spawnargs?: string[] })
            .spawnargs, // spawnargs is not in the type definition but exists at runtime
        });
