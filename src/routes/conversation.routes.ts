...
export function createConversationRoutes(
  ...
  router.post(
    '/start',
    async (
      req: Request<Record<string, never>, StartConversationResponse, StartConversationRequest> &
      ...
      logger.debug('Start conversation request', {
        ...
        body: {
          ...
          initialPrompt: req.body.initialPrompt
            ? `${req.body.initialPrompt.substring(0, 50)}...`
            : undefined,
        },
      });
  ...
  router.put(
    '/:sessionId/rename',
    async (
      req: Request<{ sessionId: string }, SessionRenameResponse, SessionRenameRequest> &
        RequestWithRequestId,
      res,
      next,
    ) => {
      ...
      try {
        ...
        if (customName.length > 200) {
          throw new CUIError(
            ...
            'customName must be 200 characters or less',
            400,
          );
        }
  ...
  router.put(
    '/:sessionId/update',
    async (
      req: Request<{ sessionId: string }, SessionUpdateResponse, SessionUpdateRequest> &
        RequestWithRequestId,
      res,
      next,
    ) => {
      ...
      try {
        ...
        if (updates.permissionMode !== undefined) {
          ...
          if (!validModes.includes(updates.permissionMode)) {
            logger.debug('Invalid permission mode', {
              requestId,
