...
import { createPodcastRoutes } from './routes/podcast.routes.js';
...
export class CUIServer {
  ...
  async stop(): Promise<void> {
    ...
    // Clean up active podcast generations
    this.logger.debug('Cleaning up podcast generations');
    try {
      const podcastfyService = PodcastfyService.getInstance(this.streamManager);
      await podcastfyService.cleanup();
    } catch (error) {
      this.logger.error('Error cleaning up podcast generations:', error);
    }
  ...
  private setupRoutes(): void {
    ...
    this.app.use('/api/podcast', createPodcastRoutes(this.streamManager));

    // Static file serving for podcasts
    this.app.use('/api/podcasts', express.static(path.join(process.cwd(), 'public/podcasts')));
