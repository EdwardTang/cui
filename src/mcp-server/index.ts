...
const CUI_SERVER_URL =
  process.env.CUI_SERVER_URL || `http://localhost:${process.env.CUI_SERVER_PORT || '3001'}`;
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    logger.debug('MCP Permission request received', {
      tool_name,
      input,
      streamingId: CUI_STREAMING_ID,
    });
      ...
      if (!permission) {
        ...
        if (!allPermissionsResponse.ok) {
          logger.error('Failed to fetch all permissions', {
            status: allPermissionsResponse.status,
          });
          throw new Error(`Failed to fetch all permissions: ${allPermissionsResponse.status}`);
        }

        const { permissions: allPermissions } =
          (await allPermissionsResponse.json()) as PermissionsResponse;
        ...
        if (processedPermission) {
          if (processedPermission.status === 'approved') {
            ...
            const denyResponse = {
              behavior: 'deny',
              message:
                processedPermission.denyReason ||
                'The user doesnt want to proceed with this tool use.The tool use was rejected(eg.if it was a file edit, the new_string was NOT written to the file).STOP what you are doing and wait for the user to tell you how to proceed.',
            };
            return {
              content: [
                {
