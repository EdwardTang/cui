  ...
  describe('edge cases', () => {
    ...
    it('should handle large event payloads', () => {
      ...
      const largeContent = Array(1000)
        ...
        .map(
          (_, i) =>
            `Line ${i}: This is a very long line with lots of content to make the payload large enough to test streaming capabilities.`,
        )
        .join('\n');
      ...
      expect(mockResponse.write).toHaveBeenCalledWith(
        expect.stringMatching(
          new RegExp(
            `^data: .*${largeContent.substring(0, 50).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}.*\\n\\n$`,
          ),
        ),
      );
    });
